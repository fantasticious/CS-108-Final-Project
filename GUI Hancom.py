"""
This GUI Hancom.py was inspired by a Korean word typing practice game. My partner and I tried to remake the game into an
English version. As the user correctly types in the falling words, the user will gain a point. As the user gains points,
the length of the falling words will increase. If the user fails to type the word before the word reaches the red line, the user
will lose a life. Losing 3 lives or reaching 50 points ends the game. 
...
class HancomApp:
    ...
    def start(self):
        ...
        app.repeat(1700, self.add_falling_word)
    ...
    def check_word(self):
        ...
        #Checks the user's points and processes the game to next stage. 
        #Moving onto stage 2
        if self.hancom.points == 5 and self.hancom.file_index == 0:
            self.next_stage()
            #Shows the user that the user has moved onto stage 2.
            self.instructions.value = 'Stage 2'
            
        #Moving onto stage 3
        if self.hancom.points == 12 and self.hancom.file_index == 1:
            self.next_stage()
            #Shows the user that the user has moved onto stage 3.
            self.instructions.value = 'Stage 3'
            
        #Moving onto stage 4
        if self.hancom.points == 22 and self.hancom.file_index == 2:
            self.next_stage()
            #Shows the user that the user has moved onto stage 4.
            self.instructions.value = 'Stage 4'
            
        #Moving onto stage 5
        if self.hancom.points == 35 and self.hancom.file_index == 3:
            self.next_stage()
            #Shows the user that the user has moved onto stage 5.
            self.instructions.value = 'Stage 5'

        #After the user reaches the final stage, the user must earn a total of 50 points to end the game.    
        if self.hancom.points == 50 and self.hancom.file_index == 4:
            self.winner_message()
